...
### 变更
- **已同步 fork 到作者仓库 `jlcodes99/cockpit-tools` 的 `28cae0d2`**：本构建包含上游 `v0.25.1` 的模型供应商切换修复、供应商协议持久化、Windows 启动修复、WebDAV 备份同步和模型目录补丁改进。
- **上游合并后继续保留本地 fork 的 CPA、自定义服务、账号排序和绑定手机号增强**：Codex API 服务仍支持内置服务、自定义配置和 CPA 服务模式；CPA 运行时管理、账号 JSON 同步、手机号导出/筛选、可配置套餐徽章和稳定自定义排序仍可使用。
