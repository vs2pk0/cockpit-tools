...
### 变更
- **已同步 fork 到作者仓库 `jlcodes99/cockpit-tools` 的 `30491dcb`**：本构建包含上游 `v0.24.11` 的 Codex API 服务账号级禁用模型规则、官方直连唤醒对话、启动凭据切换时的会话可见性修复、config.toml 保留修复和 Windows Antigravity 凭据导入更新。
...
### 新增
- **Codex 账号卡片现在按 UTC+8 显示订阅过期时间**：卡片底部原有过期时间继续保留状态颜色，并基于账号过期时间额外加 8 小时展示。
- **Codex 账号卡片现在支持绑定手机号**：卡片标签和操作按钮可打开手机号绑定弹窗，保存后在账号卡片中显示，并受页面隐私开关统一控制。

### 变更
- **已同步 fork 到作者仓库 `jlcodes99/cockpit-tools` 的 `afaa0b5d`**：本构建包含上游 `v0.24.10` 的 Codex API 服务流式对话测试、账号池健康、多实例会话同步、唤醒确认、账号级刷新间隔和打包修复。
- **上游合并后继续保留本地 fork 的 CPA/自定义服务增强**：Codex API 服务仍支持内置服务、自定义配置和 CPA 服务模式；CPA 运行时按需下载或导入，账号 JSON 同步、CPA 账号筛选、模型列表获取、配置编辑、版本更新/导入、停止确认和只识别 Cockpit 自有 CPA 进程的状态判断仍可使用。
...
### 新增
- **Codex API 服务新增 CPA 服务模式**：CPA 运行时改为按需下载或导入，不再默认打进安装包；切换到 CPA 配置后会使用端口 `8317` 和默认管理密码 `ab2026ab`，Base URL 为 `http://127.0.0.1:8317/v1`；界面会显示 CPA 启动状态，并提供 CPA 管理弹窗、配置编辑、检测更新、下载导入和停止按钮。
...
- **CPA 账号目录和 API 服务账号选择现保持同步**：CPA 模式下打开“添加账号”会读取 `~/.cli-proxy-api` JSON 中的邮箱并自动勾选；保存时新增勾选会导出 JSON，取消勾选会删除对应邮箱的 CPA JSON；账号筛选也新增 CPA 账号分类。
- **自定义服务与 CPA 服务可直接获取模型列表**：模型区域新增 `/models` 拉取按钮，内置服务专属的服务端口、访问范围和 API 端口 URL 配置仅在内置服务模式显示。

### 修复
- **CPA 服务停止现支持外部进程兜底处理并增加二次确认**：当 CPA 服务已监听端口但不属于当前前端保存的子进程句柄时，停止操作会按 `8317` 端口查找 `cli-proxy-api` 进程并停止，同时在所有停止入口弹出确认提示，避免误关正在使用的 API 服务。
- **CPA 账号筛选与管理弹窗体验优化**：CPA 账号筛选现在会同时识别 CPA 目录 JSON 和当前 CPA 服务已选账号，避免已添加账号仍显示为 `0`；CPA 服务管理弹窗扩大宽度并修复配置编辑区域的横向滚动条。
