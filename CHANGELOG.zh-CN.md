...
### 变更
- **已同步 fork 到作者仓库 `jlcodes99/cockpit-tools` 的 `cd8f1702`**：本构建包含上游 `v0.25.6` 的 Codex API 服务多协议兼容入口、大账号导入/删除/批量选择更新、配额刷新状态回写，以及 Windows Antigravity 启动修复。
...
### 变更
- **已同步 fork 到作者仓库 `jlcodes99/cockpit-tools` 的 `458be09e`**：本构建包含上游 `v0.25.4` 的 API 服务大账号量性能优化、APIKEY.FUN 暗色主题展示、WebDAV 保留天数配置、provider gateway 修复和备份恢复安全改进。
- **上游合并后继续保留本地 fork 的 CPA、自定义服务、稳定排序和绑定手机号增强**：Codex API 服务仍支持内置服务、自定义配置和 CPA 服务模式；CPA 运行时管理、账号 JSON 同步、手机号导出/筛选、可配置套餐徽章和账号自定义排序仍可使用。
