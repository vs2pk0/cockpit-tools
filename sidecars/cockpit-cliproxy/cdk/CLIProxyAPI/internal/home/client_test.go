...
func TestSubscriptionChannelsIncludesClusterWhenDiscoveryEnabled(t *testing.T) {
	client := New(config.HomeConfig{
		Enabled: true,
		Host:    "seed.example.com",
		Port:    8327,
	...
	channels := client.subscriptionChannels()

	if len(channels) != 2 {
		t.Fatalf("len(subscriptionChannels()) = %d, want 2 (%v)", len(channels), channels)
	}
	if channels[0] != redisChannelConfig {
		t.Fatalf("channels[0] = %q, want %q", channels[0], redisChannelConfig)
	}
	if channels[1] != redisChannelCluster {
		t.Fatalf("channels[1] = %q, want %q", channels[1], redisChannelCluster)
	}
}

func TestSubscriptionChannelsOmitsClusterWhenDiscoveryDisabled(t *testing.T) {
	client := New(config.HomeConfig{
		Enabled:                 true,
		Host:                    "seed.example.com",
		Port:                    8327,
	...
	if len(channels) != 1 {
		t.Fatalf("len(subscriptionChannels()) = %d, want 1 (%v)", len(channels), channels)
	}
	if channels[0] != redisChannelConfig {
		t.Fatalf("channels[0] = %q, want %q", channels[0], redisChannelConfig)
	}
