...
func (c *Client) subscriptionChannels() []string {
	channels := []string{redisChannelConfig}
	if c.clusterDiscoveryEnabled() {
		channels = append(channels, redisChannelCluster)
	}
	return channels
}

func receiveSubscriptionAcknowledgements(ctx context.Context, pubsub *redis.PubSub, expected int) error {
	if expected <= 0 {
		return nil
	}
	seen := 0
	for seen < expected {
		event, errReceive := pubsub.ReceiveTimeout(ctx, homeSubscriptionReceiveTimeout)
		if errReceive != nil {
			return errReceive
		}
		if _, ok := event.(*redis.Subscription); ok {
			seen++
		}
	}
	return nil
...
// the config channel to receive runtime config updates and, when enabled, the
// cluster channel to keep failover candidates fresh.
//
...
func (c *Client) StartConfigSubscriber(ctx context.Context, onConfig func([]byte) error) {
	...
	for {
		...
		channels := c.subscriptionChannels()
		...
		if errReceive := receiveSubscriptionAcknowledgements(ctx, pubsub, len(channels)); errReceive != nil {
			_ = pubsub.Close()
			c.markReconnectFailure("subscribe")
			sleepWithContext(ctx, homeReconnectInterval)
			continue
		...
		for {
			event, errMsg := pubsub.Receive(ctx)
			if errMsg != nil {
				_ = pubsub.Close()
				c.heartbeatOK.Store(false)
